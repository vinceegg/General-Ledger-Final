...
class GrantsDonationsinKindShow extends Component
{
    ...
    {       
        ...
        GrantsDonationsinKindModel::create($validatedData);
    ...
    {
        $general_ledger = GrantsDonationsinKindModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        GrantsDonationsinKindModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new GrantsDonationsinKindImport, $filePath);

        return redirect()->route('GrantsDonationsinKind')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new GrantsDonationsinKindExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new GrantsDonationsinKindExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = GrantsDonationsinKindModel::query();
        ...
        $grants_donations_in_kind = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.grants-donationsin-kind-show',['general_ledger' => $grants_donations_in_kind]);
    }
}
