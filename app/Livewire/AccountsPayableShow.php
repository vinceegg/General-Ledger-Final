...
class AccountsPayableShow extends Component
{
    ...
    {       
        ...
        AccountsPayableModel::create($validatedData);
    ...
    {
        $general_ledger = AccountsPayableModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        AccountsPayableModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new AccountsPayableImport, $filePath);

        return redirect()->route('AccountsPayable')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new AccountsPayableExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new AccountsPayableExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = AccountsPayableModel::query();
        ...
        $accounts_payable = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.accounts-payable-show',['general_ledger' => $accounts_payable]);
    }
}
