...
class RepresentationAllowanceShow extends Component
{
    ...
    {       
        ...
        RepresentationAllowanceModel::create($validatedData);
    ...
    {
        $general_ledger = RepresentationAllowanceModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        RepresentationAllowanceModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new RepresentationAllowanceImport, $filePath);

        return redirect()->route('RepresentationAllowance')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new RepresentationAllowanceExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new RepresentationAllowanceExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = RepresentationAllowanceModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = RepresentationAllowanceModel::query();
        ...
        $representation_allowance = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.representation-allowance-show',['general_ledger' => $representation_allowance]);
    }
}
