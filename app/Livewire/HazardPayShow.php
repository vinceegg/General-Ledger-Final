...
class HazardPayShow extends Component
{
    ...
    {       
        ...
        HazardPayModel::create($validatedData);
    ...
    {
        $general_ledger = HazardPayModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        HazardPayModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new HazardPayImport, $filePath);

        return redirect()->route('HazardPay')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new HazardPayExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new HazardPayExport, 'Ledger Sheet.csv');
    }
    ...
    public function render()
    {
        ...
        $hazard_pay = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.hazard-pay-show',['general_ledger' => $hazard_pay]);
    }
}
