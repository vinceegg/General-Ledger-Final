...
class SportsEquipmentShow extends Component
{
    ...
    {       
        ...
        SportsEquipmentModel::create($validatedData);
    ...
    {
        $general_ledger = SportsEquipmentModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        SportsEquipmentModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new SportsEquipmentImport, $filePath);

        return redirect()->route('SportsEquipment')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new SportsEquipmentExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new SportsEquipmentExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = SportsEquipmentModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = SportsEquipmentModel::query();
        ...
        $sports_equipment = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.sports-equipment-show',['general_ledger' => $sports_equipment]);
    }
}
