...
class DuetoGSISShow extends Component
{
    ...
    {       
        ...
        DuetoGSISModel::create($validatedData);
    ...
    {
        $general_ledger = DuetoGSISModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        DuetoGSISModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new DuetoGSISImport, $filePath);

        return redirect()->route('DuetoGSIS')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new DuetoGSISExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new DuetoGSISExport, 'Ledger Sheet.csv');
    }
    ...
    public function render()
    {
        ...
        $due_to_gsis = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.dueto-g-s-i-s-show',['general_ledger' => $due_to_gsis]);
    }
}
