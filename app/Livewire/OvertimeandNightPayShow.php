...
class OvertimeandNightPayShow extends Component
{
    ...
    {       
        ...
        OvertimeandNightPayModel::create($validatedData);
    ...
    {
        $general_ledger = OvertimeandNightPayModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        OvertimeandNightPayModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new OvertimeandNightPayImport, $filePath);

        return redirect()->route('OvertimeandNightPay')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new OvertimeandNightPayExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new OvertimeandNightPayExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = OvertimeandNightPayModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = OvertimeandNightPayModel::query();
        ...
        $overtime_and_night_pay = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.overtimeand-night-pay-show',['general_ledger' => $overtime_and_night_pay]);
    }
}
