...
class WaterExpensesShow extends Component
{
    ...
    {       
        ...
        WaterExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = WaterExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        WaterExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new WaterExpensesImport, $filePath);

        return redirect()->route('WaterExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new WaterExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new WaterExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = WaterExpensesModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = WaterExpensesModel::query();
        ...
        $water_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.water-expenses-show',['general_ledger' => $water_expenses]);
    }
}
