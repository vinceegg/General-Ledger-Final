...
class PhilHealthContributionsShow extends Component
{
    ...
    {       
        ...
        PhilHealthContributionsModel::create($validatedData);
    ...
    {
        $general_ledger = PhilHealthContributionsModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        PhilHealthContributionsModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new PhilHealthContributionsImport, $filePath);

        return redirect()->route('PhilHealthContributions')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new PhilHealthContributionsExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new PhilHealthContributionsExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = PhilHealthContributionsModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = PhilHealthContributionsModel::query();
        ...
        $philhealth_contributions = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.phil-health-contributions-show',['general_ledger' => $philhealth_contributions]);
    }
}
