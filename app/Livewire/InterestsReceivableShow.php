...
class InterestsReceivableShow extends Component
{
    ...
    {       
        ...
        InterestsReceivableModel::create($validatedData);
    ...
    {
        $general_ledger = InterestsReceivableModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        InterestsReceivableModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new InterestsReceivableImport, $filePath);

        return redirect()->route('InterestsReceivable')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new InterestsReceivableExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new InterestsReceivableExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = InterestsReceivableModel::query();
        ...
        $interests_receivable = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.interests-receivable-show',['general_ledger' => $interests_receivable]);
    }
}
