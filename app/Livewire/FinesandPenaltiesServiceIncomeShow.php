...
class FinesandPenaltiesServiceIncomeShow extends Component
{
    ...
    {       
        ...
        FinesandPenaltiesServiceIncomeModel::create($validatedData);
    ...
    {
        $general_ledger = FinesandPenaltiesServiceIncomeModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        FinesandPenaltiesServiceIncomeModel::where('id', $this->general_ledger_id)->update([
    ...
    {
        return Excel::download(new FinesandPenaltiesServiceIncomeExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new FinesandPenaltiesServiceIncomeExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = FinesandPenaltiesServiceIncomeModel::query();
        ...
        $fines_and_penalties_service_income = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.finesand-penalties-service-income-show',['general_ledger' => $fines_and_penalties_service_income]);
    }
}
