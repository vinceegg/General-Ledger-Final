...
class DrugsandMedicinesExpensesShow extends Component
{
    ...
    {       
        ...
        DrugsandMedicinesExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = DrugsandMedicinesExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        DrugsandMedicinesExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new DrugsandMedicinesExpensesImport, $filePath);

        return redirect()->route('DrugsandMedicinesExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new DrugsandMedicinesExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new DrugsandMedicinesExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = DrugsandMedicinesExpensesModel::query();
        ...
        $drugs_and_medicines_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.drugsand-medicines-expenses-show',['general_ledger' => $drugs_and_medicines_expenses]);
    }
}
