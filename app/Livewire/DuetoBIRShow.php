...
class DuetoBIRShow extends Component
{
    ...
    {       
        ...
        DuetoBIRModel::create($validatedData);
    ...
    {
        $general_ledger = DuetoBIRModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        DuetoBIRModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new DuetoBIRImport, $filePath);

        return redirect()->route('DuetoBIR')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new DuetoBIRExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new DuetoBIRExport, 'Ledger Sheet.csv');
    }
    ...
    public function render()
    {
        ...
        $due_to_bir = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.dueto-b-i-r-show',['general_ledger' => $due_to_bir]);
    }
}
