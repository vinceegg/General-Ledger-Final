...
class DuetoPAGIBIGTrash extends Component
{
    ...
    {
        $this->softDeletedData = DuetoPAGIBIGModel::onlyTrashed()->get();
    }

    // Method to restore soft-deleted record
    public function restoreGeneralLedger($id)
    {
        $general_ledger = DuetoPAGIBIGModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $general_ledger = DuetoPAGIBIGModel::withTrashed()->find($this->general_ledger_id);
        ...
        $this->resetInput();
        return redirect()->route('DuetoPAGIBIGArchived')->with('message', 'Deleted Successfully');
    }
