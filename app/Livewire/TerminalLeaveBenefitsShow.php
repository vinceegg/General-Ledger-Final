...
class TerminalLeaveBenefitsShow extends Component
{
    ...
    {       
        ...
        TerminalLeaveBenefitsModel::create($validatedData);
    ...
    {
        $general_ledger = TerminalLeaveBenefitsModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        TerminalLeaveBenefitsModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new TerminalLeaveBenefitsImport, $filePath);

        return redirect()->route('TerminalLeaveBenefits')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new TerminalLeaveBenefitsExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new TerminalLeaveBenefitsExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = TerminalLeaveBenefitsModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = TerminalLeaveBenefitsModel::query();
        ...
        $terminal_leave_benefits = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.terminal-leave-benefits-show',['general_ledger' => $terminal_leave_benefits]);
    }
}
