...
class RentIncomeShow extends Component
{
    ...
    {       
        ...
        RentIncomeModel::create($validatedData);
    ...
    {
        $general_ledger = RentIncomeModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        RentIncomeModel::where('id', $this->general_ledger_id)->update([
    ...
    {
        return Excel::download(new RentIncomeExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new RentIncomeExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = RentIncomeModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    public function render()
    {
        ...
        $rent_income = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.rent-income-show',['general_ledger' => $rent_income]);
    }
}
