...
class PrintingandPublicationExpensesShow extends Component
{
    ...
    {       
        ...
        PrintingandPublicationExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = PrintingandPublicationExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        PrintingandPublicationExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new PrintingandPublicationExpensesImport, $filePath);

        return redirect()->route('PrintingandPublicationExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new PrintingandPublicationExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new PrintingandPublicationExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = PrintingandPublicationExpensesModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = PrintingandPublicationExpensesModel::query();
        ...
        $printing_and_publication_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.printingand-publication-expenses-show',['general_ledger' => $printing_and_publication_expenses]);
    }
}
