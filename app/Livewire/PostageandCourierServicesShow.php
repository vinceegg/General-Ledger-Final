...
class PostageandCourierServicesShow extends Component
{
    ...
    {       
        ...
        PostageandCourierServicesModel::create($validatedData);
    ...
    {
        $general_ledger = PostageandCourierServicesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        PostageandCourierServicesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new PostageandCourierServicesImport, $filePath);

        return redirect()->route('PostageandCourierServices')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new PostageandCourierServicesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new PostageandCourierServicesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = PostageandCourierServicesModel::query();
        ...
        $postage_and_courier_services = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.postageand-courier-services-show',['general_ledger' => $postage_and_courier_services]);
    }
}
