...
class PriorPeriodAdjustmentShow extends Component
{
    ...
    {       
        ...
        PriorPeriodAdjustmentModel::create($validatedData);
    ...
    {
        $general_ledger = PriorPeriodAdjustmentModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        PriorPeriodAdjustmentModel::where('id', $this->general_ledger_id)->update([
    ...
    {
        return Excel::download(new PriorPeriodAdjustmentExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new PriorPeriodAdjustmentExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = PriorPeriodAdjustmentModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = PriorPeriodAdjustmentModel::query();
        ...
        $prior_period_adjustment = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.prior-period-adjustment-show',['general_ledger' => $prior_period_adjustment]);
    }
}
