...
use App\Exports\LS2PettyCashExport;
use App\Imports\LS2PettyCashImport;
...
class LS2PettyCashShow extends Component
{
    use WithFileUploads;
    ...
    {       
        ...
        LS2PettyCashModel::create($validatedData);
    ...
    {
        $general_ledger = LS2PettyCashModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        LS2PettyCashModel::where('id', $this->general_ledger_id)->update([
    ...
    {
        return Excel::download(new LS2PettyCashExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new LS2PettyCashExport, 'Ledger Sheet.csv');
    }
    ...
    public function render()
    {
        ...
        $ls2_petty_cash = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.l-s2-petty-cash-show',['general_ledger' => $ls2_petty_cash]);
    }
}
