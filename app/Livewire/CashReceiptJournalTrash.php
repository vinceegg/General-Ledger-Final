...
class CashReceiptJournalTrash extends Component
{
    ...
    {
        $general_ledger = CashReceiptJournalModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $general_ledger = CashReceiptJournalModel::withTrashed()->find($this->general_ledger_id);
        ...
        $this->resetInput();
        return redirect()->route('CRJ')->with('message', 'Deleted Successfully');
    }
