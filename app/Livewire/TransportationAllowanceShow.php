...
class TransportationAllowanceShow extends Component
{
    ...
    {       
        ...
        TransportationAllowanceModel::create($validatedData);
    ...
    {
        $general_ledger = TransportationAllowanceModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        TransportationAllowanceModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new TransportationAllowanceImport, $filePath);

        return redirect()->route('TransportationAllowance')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new TransportationAllowanceExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new TransportationAllowanceExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = TransportationAllowanceModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = TransportationAllowanceModel::query();
        ...
        $transportation_allowance = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.transportation-allowance-show',['general_ledger' => $transportation_allowance]);
    }
}
