...
class TelephoneExpensesShow extends Component
{
    ...
    {       
        ...
        TelephoneExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = TelephoneExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        TelephoneExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new TelephoneExpensesImport, $filePath);

        return redirect()->route('TelephoneExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new TelephoneExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new TelephoneExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = TelephoneExpensesModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = TelephoneExpensesModel::query();
        ...
        $telephone_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.telephone-expenses-show',['general_ledger' => $telephone_expenses]);
    }
}