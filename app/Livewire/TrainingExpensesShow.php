...
class TrainingExpensesShow extends Component
{
    ...
    {       
        ...
        TrainingExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = TrainingExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        TrainingExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new TrainingExpensesImport, $filePath);

        return redirect()->route('TrainingExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new TrainingExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new TrainingExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = TrainingExpensesModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = TrainingExpensesModel::query();
        ...
        $training_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.training-expenses-show',['general_ledger' => $training_expenses]);
    }
}
