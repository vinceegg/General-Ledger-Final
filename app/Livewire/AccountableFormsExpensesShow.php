...
class AccountableFormsExpensesShow extends Component
{
    ...
    {       
        ...
        AccountableFormsExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = AccountableFormsExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        AccountableFormsExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new AccountableFormsExpensesImport, $filePath);

        return redirect()->route('AccountableFormsExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new AccountableFormsExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new AccountableFormsExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = AccountableFormsExpensesModel::query();
        ...
        $accountable_forms_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.accountable-forms-expenses-show',['general_ledger' => $accountable_forms_expenses]);
    }
}
