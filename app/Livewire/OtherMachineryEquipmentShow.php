...
class OtherMachineryEquipmentShow extends Component
{
    ...
    {       
        ...
        OtherMachineryEquipmentModel::create($validatedData);
    ...
    {
        $general_ledger = OtherMachineryEquipmentModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        OtherMachineryEquipmentModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new OtherMachineryEquipmentImport, $filePath);

        return redirect()->route('OtherMachineryEquipment')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new OtherMachineryEquipmentExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new OtherMachineryEquipmentExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = OtherMachineryEquipmentModel::query();
        ...
        $other_machinery_equipment = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.other-machinery-equipment-show',['general_ledger' => $other_machinery_equipment]);
    }
}
