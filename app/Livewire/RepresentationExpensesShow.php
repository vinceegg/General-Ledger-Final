...
class RepresentationExpensesShow extends Component
{
    ...
    {       
        ...
        RepresentationExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = RepresentationExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        RepresentationExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    {
        return Excel::download(new RepresentationExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new RepresentationExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = RepresentationExpensesModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = RepresentationExpensesModel::query();
        ...
        $representation_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.representation-expenses-show',['general_ledger' => $representation_expenses]);
    }
}
