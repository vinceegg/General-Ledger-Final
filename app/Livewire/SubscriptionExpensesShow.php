...
class SubscriptionExpensesShow extends Component
{
    ...
    {       
        ...
        SubscriptionExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = SubscriptionExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        SubscriptionExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new SubscriptionExpensesImport, $filePath);

        return redirect()->route('SubscriptionExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new SubscriptionExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new SubscriptionExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = SubscriptionExpensesModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = SubscriptionExpensesModel::query();
        ...
        $subscription_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.subscription-expenses-show',['general_ledger' => $subscription_expenses]);
    }
}
