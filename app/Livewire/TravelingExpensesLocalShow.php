...
class TravelingExpensesLocalShow extends Component
{
    ...
    {       
        ...
        TravelingExpensesLocalModel::create($validatedData);
    ...
    {
        $general_ledger = TravelingExpensesLocalModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        TravelingExpensesLocalModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new TravelingExpensesLocalImport, $filePath);

        return redirect()->route('TravelingExpensesLocal')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new TravelingExpensesLocalExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new TravelingExpensesLocalExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = TravelingExpensesLocalModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = TravelingExpensesLocalModel::query();
        ...
        $traveling_expenses_local = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.traveling-expenses-local-show',['general_ledger' => $traveling_expenses_local]);
    }
}
