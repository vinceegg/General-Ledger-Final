...
class CashGiftShow extends Component
{
    ...
    {       
        ...
        CashGiftModel::create($validatedData);
    ...
    {
        $general_ledger = CashGiftModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        CashGiftModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new CashGiftImport, $filePath);

        return redirect()->route('CashGift')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new CashGiftExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new CashGiftExport, 'Ledger Sheet.csv');
    }
    ...
    public function render()
    {
        ...
        $cash_gift = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.cash-gift-show',['general_ledger' => $cash_gift]);
    }
}
