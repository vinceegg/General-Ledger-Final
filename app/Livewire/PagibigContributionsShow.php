...
class PagibigContributionsShow extends Component
{
    ...
    {       
        ...
        PagibigContributionsModel::create($validatedData);
    ...
    {
        $general_ledger = PagibigContributionsModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        PagibigContributionsModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new PagibigContributionsImport, $filePath);

        return redirect()->route('PagibigContributions')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new PagibigContributionsExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new PagibigContributionsExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $general_ledger = PagibigContributionsModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $query = PagibigContributionsModel::query();
        ...
        $pag_ibig_contributions = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.pagibig-contributions-show',['general_ledger' => $pag_ibig_contributions]);
    }
}
