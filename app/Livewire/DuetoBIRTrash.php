...
class DuetoBIRTrash extends Component
{
    ...
    {
        $this->softDeletedData = DuetoBIRModel::onlyTrashed()->get();
    }

    // Method to restore soft-deleted record
    public function restoreGeneralLedger($id)
    {
        $general_ledger = DuetoBIRModel::onlyTrashed()->find($id);
        if ($general_ledger) {
    ...
    {
        $general_ledger = DuetoBIRModel::withTrashed()->find($this->general_ledger_id);
        ...
        $this->resetInput();
        return redirect()->route('DuetoBIRArchived')->with('message', 'Deleted Successfully');
    }
