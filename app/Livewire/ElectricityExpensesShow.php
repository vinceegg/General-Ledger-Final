...
class ElectricityExpensesShow extends Component
{
    ...
    {       
        ...
        ElectricityExpensesModel::create($validatedData);
    ...
    {
        $general_ledger = ElectricityExpensesModel::find($general_ledger_id);
        if ($general_ledger) {
    ...
    public function updateGeneralLedger()
    {
        ...
        ElectricityExpensesModel::where('id', $this->general_ledger_id)->update([
    ...
    // Ensure that a file has been uploaded
        ...
        Excel::import(new ElectricityExpensesImport, $filePath);

        return redirect()->route('ElectricityExpenses')->with('message', 'File Imported Successfully');
        }
    }
    ...
    {
        return Excel::download(new ElectricityExpensesExport, 'Ledger Sheet.xlsx');
    }
    public function exportGl_CSV(Request $request) 
    {
        return Excel::download(new ElectricityExpensesExport, 'Ledger Sheet.csv');
    }
    ...
    {
        $query = ElectricityExpensesModel::query();
        ...
        $electricity_expenses = $query->orderBy('id', 'ASC')->get(); // Changed from paginate() to get()
        ...
        return view('livewire.electricity-expenses-show',['general_ledger' => $electricity_expenses]);
    }
}
