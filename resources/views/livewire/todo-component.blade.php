            ...
            @foreach ($todos as $todo)         
                <div class="{{ $todo->isDone ? '' : '' }} py-4 flex items-center border-b border-gray-300 px-3">
                    ...
                    <div class="flex space-x-3">
                        <button wire:click="update({{ $todo->id }})" class="py-2 px-2 bg-green-500 hover:bg-green-600 text-white rounded-md">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" />
                            </svg>
                        </button>
                        <button wire:click="destroy({{ $todo->id }})" class="py-2 px-2 bg-red-500 hover:bg-red-600 text-white rounded-md">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
